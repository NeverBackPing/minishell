...
char	*check_str(char *buf)
{
	...
	if (!buf)
		return (NULL);
	token = (char *)malloc((ft_strlen(buf) + 1) * sizeof(char));
	if (token == NULL)
		return (NULL);
	ft_strlcpy(token, buf, ft_strlen(buf) + 1);
	return (token);
}
...
void	change_delim(char *buf, size_t pos)
{
	if (buf[--pos] != '/' )
		buf[++pos] = '/';
}
...
char	*ft_strtok(char *str, const char delimiter)
{
	...
	while (pos < ft_strlen(buf))
	{
		if (buf[pos] == delimiter)
			return (change_delim(token, i), pos++, token);
		token[i++] = buf[pos++];
	}
	return (token);
}
